...
Out-of-bag estimate of misclassification error:  0.1633 
\end{Soutput}
...
\end{Sinput}
\begin{Soutput}
[1] normal   normal   normal   glaucoma glaucoma glaucoma
[7] glaucoma
...
\begin{Sinput}
>fit <- inclass(formula.indirect, pFUN = lm, data = GlaucomaMVF)
\end{Sinput}
...
+         clv >= 5.1 & lora >= 49.23372) | (!is.na(clv) & 
+         !is.na(lora) & !is.na(cs) & clv < 5.1 & 
+         lora >= 58.55409 & cs < 1.405) | (is.na(clv) & 
+         !is.na(lora) & !is.na(cs) & lora >= 58.55409 & 
...
\begin{Sinput}
>predict(object = fit, cFUN = classify, newdata = GlaucomaMVF[c(1:3, 
+     86:88), ])
\end{Sinput}
\begin{Soutput}
[1] normal   normal   normal   glaucoma glaucoma glaucoma
...
\texttt{est.para = list(k = 10)}. The options \texttt{estimator = "boot"} or 
...
\begin{Sinput}
>errorest(Class ~ ., data = GlaucomaM, model = lda, 
...
Misclassification error:  0.2347 
\end{Soutput}
...
+     res <- predict.inclass(object = object, cFUN = classify, 
+         newdata = newdata)
+     return(res)
+ }
\end{Sinput}
...
+     estimator = "632plus", iclass = "Class", pFUN = lm)
\end{Sinput}
\begin{Soutput}
Call:
...
Misclassification error:  0.2582 
\end{Soutput}
