...
predict(object = fit, cFUN = classify, newdata = GlaucomaMVF[c(1:3, 86:88),])
@
...
fit <- inclass(formula.indirect, pFUN = bagging, nbagg = 25, 
data = GlaucomaMVF)
fit
@
...
mypredict.inclass <- function(object, newdata){
  res <- predict.inclass(object = object, cFUN = classify, newdata = newdata)
  return(res)
}
@
